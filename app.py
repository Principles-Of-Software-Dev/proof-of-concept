...
class formSignup(FlaskForm):
    firstname = StringField(label='First', validators=[InputRequired()])
    lastname = StringField(label='Last', validators=[InputRequired()])
    dob = DateField(label='Date of Borth', validators=[InputRequired()])
    email = StringField(label='Email', validators=[InputRequired(), Email()])
    ...
    if form.validate_on_submit and request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                login_user(user)
                flash("You have been successfully logged in!", "alert-success")
                return redirect('/dashboard', 302)
        else:
